...
func ApiServer() {
	...
	r.HandleFunc("ok", StatusPing).Methods("GET")
	http.Handle("/", r)
	http.ListenAndServe(fmt.Sprintf(":%v", *httpPort), nil)
}

func StatusPing(w http.ResponseWriter, r *http.Request) {
	log.Println("Received a request to check status.")
	w.WriteHeader(http.StatusCreated)
	fmt.Fprintf(w, "{ \"status\": \"maybe\" }")
}
